...
gbreal2guass = nl.inv( np.exp(-alpha * (xg[:,np.newaxis] - xg)**2) )
...
print('Objective for predicted theta: ', objecfn(resform))
# Relative norm difference of predicted theta to true theta
print('RND predicted theta vs. true theta: ', nl.norm(resform - gbcfftrue) / nl.norm(gbcfftrue))
...
print('RND predicted potential vs. true potential: ', nl.norm(vtrue - vformprdc))
# Relative norm difference of the predicted potential to training data
print('RND predicted potential vs. training potential: ', nl.norm(vtruerecon - vformprdc) / nl.norm(vtruerecon))
# plot potentials
trim = 1
...
print('RND predicted Psi vs. test Psi:', nl.norm(psimatformprdc - psimattruetest) /  nl.norm(psimattruetest))
# plot the real part of the final state
plt.plot(xvec, psimatformprdc[-1].real, 'r', label='Predicted')
plt.plot(xvec, psimattruetest[-1].real, 'k', label='Test')
plt.legend()
plt.title('Real Part of the Final State')
plt.xlabel('Position')
plt.ylabel('Magnitude')
plt.show()
# plot the imaginary part of the final state
plt.plot(xvec, psimatformprdc[-1].imag, 'r', label='Predicted')
plt.plot(xvec, psimattruetest[-1].imag, 'k', label='Test')
plt.legend()
plt.title('Imaginary Part of the Final State')
plt.xlabel('Position')
plt.ylabel('Magnitude')
plt.show()
